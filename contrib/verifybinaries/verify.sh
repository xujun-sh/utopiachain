...
###   utopiacoincore.org and utopiacoin.org and compares them.
...
WORKINGDIR="/tmp/utopiacoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://utopiacoincore.org"
HOST2="https://utopiacoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="utopiacoin-core-"
RCVERSIONSTRING="rc"
...
if [ $? -ne 0 ]; then
   echo "utopiacoin.org failed to provide signature file, but utopiacoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "utopiacoin.org and utopiacoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Utopiacoin Core binary release signing key installed?"
   fi
