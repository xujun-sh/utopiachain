...
The RPM spec file provided here is for Utopiacoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://utopiacoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Utopiacoin git repository](https://github.com/utopiacoin/utopiacoin) but are not part of
...
Currently only `Source10` is used, for the example `utopiacoin.conf` file.
...
in the utopiacoin git repository. At present only an SVG version of the Utopiacoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the utopiacoin
...
needed when 0.12.1 is released, a proper fix is already in the Utopiacoin git
master branch.
...
idea to build Utopiacoin Core against a static version of that library compiled
...
The `utopiacoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `utopiacoin-server` package. At this point, most RPM based
...
The `utopiacoin-server` package should have SELinux support. How to properly do
...
Utopiacoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/utopiacoin/). He has been
packaging Utopiacoin for Fedora at least since 2012.
...
4. I renamed the `utopiacoin` package that contains the Qt GUI to `utopiacoin-core` as
that appears to be how the general population refers to it, in contrast to
`utopiacoin-xt` or `utopiacoin-classic`. I wanted to make sure the general population
